...
- 구조적 설계
  ...
  - 변환 흐름 중심 설계
    - 입력 흐름 : 입력을 받아들여 시스템에 사용할 수 있는 데이터로 정제하는 일 
    - 변환 중심 : 데이터 가공처리 기능 담당, 계산을 수행하는 대부분의 프로세스 포함
    - 출력 흐름 : 가공 처리된 정보를 받아 적당 출력물로 변환시켜 출력
  - 프로그램 구조 전환 가이드라인
    1. 요구사항 명세서로 부터 정제된 자료흐름도를 만든다. 
    2. 자료흐름의 유형을 조사한다.
    3. 입력 경계와 출력 경계를 정한다.
      - 입력 흐름과 관계되는 프로세스들을 규명
      - 출력 흐름과 관계되는 프로세스들 구셤
      - 입력 흐름에 속한 프로세스와 출력 흐름에 속한 프로세스들 규명
    4. 최상위 수준에서 자료흐름 중심 프로그램 구조 개발
      - 변환 흐름 중심 자료흐름도에서 최상위 프로그램 구조의 3가지 구성요소
        - 입력 제어 모듈 
        - 출력 제어 모듈
        - 변환 제어 모듈
        
        ![ex_screenshot](/res/설계1.JPG)
        ![ex_screenshot](/res/설계2.JPG)
    5. 자료흐름도를 프로그램 구조로 전환 ( 두번째 수준의 세분화 )
      ![ex_screenshot](/res/설계3.JPG)
    6. 프로그램 구조를 정제  
  - 트랜잭션 흐름 중심 설계
    - 들어온 입력을 여러 갈래의 출력 흐름으로 쪼갤 수 있는 경우에 가능
    - 정보 흐름 중심 = 트랜잭션 중심
    ![ex_screenshot](/res/설계4.JPG)
  - 전환 가이드라인
    1. 요구사항 명세서로부터 하나의 자료흐름도를 만듬
    2. 자료흐름도가 트랜잭션 흐름을 갖는지 결정
    3. 트랜잭션 중심과 동작 경로를 파악
    4. 트랜잭션 흐름 프로그램 구조를 개발
      - 트랜잭션 흐름 중심 자료흐름도에서 3가지 구성요소
        - 트랜잭션 중심으로 작용하는 모듈
        - 입력을 받아들이는 모듈
        - 각 동작 경로에 해당하는 하나 이상의 모듈
    5. 자료흐름도를 프로그램 구조로 매핑
      
      ![ex_screenshot](/res/설계5.JPG)
    6. 프로그램 구조 정제

- 사용자 인터페이스 설계
  - 사용자 인터페이스에 대한 조사는 요구사항 분석단계부터 이루어짐
  - 구체적인 사용자 인터페이스에 대한 정의는 기본 설계과정에서 이루어지는 것이 일반적
  - 기본 설계 과정에서 사용자 인터페이스와 소프트웨어 구조 설계(대화모형도, 처리모형도)
  - 사용자에게 무엇을 보여주는가의 문제가 중요!
  - 사용자 안내서 : 인터페이스에 대한 구체적 설명 기록 
  - 시스템의 모든 사용자 인터페이스는 일관성을 유지해야한다. 
- 상세설계
  - 순서도 : 그래픽 요소로 알고리즘 제어 흐름을 표시하는 그래픽 도구
    - 간단하고 표현력이 높으며 배우기 쉽다.
    - 간단한 모듈의 내부 구조나 동작 표현에는 적합하나 복잡한 시스템 구조를 설명하는 것을 붖거합
  - N-S 도표 : 순서도의 비구조적 단점을 극복하고자 나온 그래픽 프로그램 표기법
    - 프로그램 기본 제어 구조를 서로 다른 도표로 표시하여 쉽게 읽고 이해
    - 순환 제어 구조를 쉽게 표시
    ![ex_screenshot](/res/NS도표.JPG)
  - 프로그램 설계 언어 (PDL, 가상 코드)
    - 고급 프로그램 언어의 제어 구조와 자연 언어를 통합하여 사용할 수 있는 알고리즘 표기 언어

### 소프트웨어 구현
- 프로그래밍 : 설계의 연장
  - 설계 명세서에 기술된 결과를 구체화하여 컴퓨터가 이해할 수 있는 모습으로 바꾸는 것
  - 코딩 : 설계가 제대로 이루어져 프로그래밍이 기계적으로 이루어 지는 것
- 프로그램(코드) 문서화 : 프로그램이 무엇을 수행하고 어떻게 동작하는지 독자가 이해할 수 있게 작성된 일련의 기술서
  - 내부 문서화 : 코드와 관련된 기술서, 프로그램 특성, 제어 구조등 요약정보를 기술해 소스코드 읽는 사람에게 제공
  - 외부 문서화 : 실제 코드를 한번도 보지 않는 사람에게 시스템 입장에서 육하원칙으로 구성
  - 프로그램이 만들어진 후, 설계 명세서를 참조하지 않고도 모듈 기능이 쉽게 이해될 수 있어야함.
  - 변수, 레이블 이름은 정직하게, 시별자 이름 선정은 프로그램 이해에 중요한 역할
- 코드 검사
  - 소프트웨어의 오류를 발견하여 제거함으로써 높은 품질의 소프트웨어를 얻기 위한 활동
  - 개발 과정에서 요구되는 공식 기술 검토중 하나, 코드에 대한 시험 이전에 이루어짐
  - 개인에 의해가 아닌 조직에 의해 이루어짐을 보여주는 것
  - 품질 보증 활동의 일부로 인식
  - 프로젝트와 관계있는 동료 프로그래머들에 의해 수행되는 것이 일반적 
  - 오류를 지적하는 것이지 해결방법을 찾으려는 것이 아니며, 최소한도의 시간이 소요되도록 함

### 소프트웨어 시험
- 소프트웨어 품질 확보 및 결함을 찾아내기 위해 수행되는 일련의 작업
- 소프트웨어 품질 보증을 위한 마지막 단계, 보루!!
- 시험은 오류를 발견하려고 프로그램을 실행시키는 것이다
- 목적
  - 소프트웨어 내 존재하는 오류 발견
  - 소프트웨어 요구사항에 충족하는지 확인
  - 소프트웨어 명세에 충족하는지 확인
  - 출시 이후 발생할 수 있는 결함 예방
  
![ex_screenshot](/res/시험.JPG)

- V모델 소프트웨어 테스트 단계
  - 단위 시험 : 프로그램 기본 단위인 모듈에 대한 시험
    - 설계 단계의 모듈에 대한 설계 지침을 기반으로 모듈의 중요한 경로와 경계값 시험
    - 시험 프로세스의 첫 단계
    - 코드가 효율적으로 작성 되었는지, 코딩 표준을 준수하였는지 검증
    - 모듈의 인터페이스, 자료구조, 오류처리에 대한 검증
  - 통합 시험 : 각 모듈들을 통합하여 통합된 컴포넌트 간 인터페이스와 상호작용 상 오류를 발견하는 작업 수행
    - 블랙박스 시험을 통해 확인하며, 기본 설계에서 준비된 시험 사례를 사용해 진행
    - 점진적인 통합 시험을 실시(하향식 / 상향식)
    - 회귀 시험
      - 새로운 모듈의 추가 및 변경으로 시스템 변경 발생에 따른 영향으로 기존 시험을 재수행할 필요가 있을 때 수행하는 시험으로 기존 시험 사례를 이용해 재시험 수행
      - 회귀 시험 사례 선정 고려사항
        - 소프트웨어 전체 기능을 수행할 수 있는 사례
        - 새로운 모듈에 의해 영향 받을 수 있는 기능에 대한 사례
        - 변경된 소프트웨어 구성 요소에 대해 집중적으로 시험 가능한 사례
  - 시스템 시험 : 소프트웨어와 다른 시스템 요소들과 통합, 모든 요소들이 적절히 조화 이루어 시스템 기능을 만족하는지 확인하는 시험
    - 실제 구현 시스템과 계획된 사양을 서로 비교하는 작업 수행
    - 기능을 시험 확인하고 성능이나 기능에 제한이 없는지 확인
    - 복구 시험 / 보안 시험 / 스트레스 시험 / 성능 시험
  - 인수 시험 : 개발된 소프트웨어가 고객의 요구사항을 만족하는지 조사하는 확ㅇ니 시험
    - 사용자가 수행하는 확인, 고객 요구사항 만족 여부를 검증하는 것
    - a-시험 : 사용자가 개발 환경에 와서 시험하는 것
    - b-시험 : 개발팀이 소프트웨어를 사용자에 배포하여 사용자가 자신의 컴퓨터 환경, 실제 상황에서 수행하는 시험
- 소프트웨어 테스트 방법 및 테스트 설계 기법
  - 블랙박스 시험 : 내부 구조를 참조하지 않고 주어진 입력에 요구되는 결과가 나오는지 시험
    - 프로그램과 외부와의 인터페이스에 대해 시험
    - 프로그램의 기능에 대해 초점
    - 동등 분할 / 경계값 분석 / 페어와이즈 조합 / 상태전이 / 인과 그래프 / 결정 테이블 등...
    - 동등 분할 : 오류 종류에 따라 프로그램의 입력 데이터를 분류하여 사례를 설계하는 것
    - 경계값 분석 : 입력 조건의 경계값 주위의 입력값을 시험 데이터로 선정 
      - 오류를 발견할 가능성이 높음..
  - 화이트 박스 시험 : 프로그램 내부의 구성을 시험하는 것
    - 프로그램을 참조하여 이루어지며, 구조 또는 코드에 기초한 시험
    - 모든 실행문 커버지리 : 수행 가능 문장을 한번은 실행되도록 설계하며 요구 결고가 나오는지 검증
    - 분기/결정 커버리지, 변경조건/결정 검증기준, 다중조건, 기본 경로 시험, 자료흐름 시험
    - 조건 커버리지 : 충분한 사례를 설계하여 조건문의 모든 조건식을 만족하는 경우, 아닌경우 시험
  - 블랙박스 시험의 문제점
    - 모든 실행문이 실제 수행되는지 알수 없다.
    - 부작용 예측이 어렵고 모듈이 실행되어 전역 변수 값을 변하게 할 수도 있다
  - 화이트박스 시험 필요성 
    - 프로그램 내 불필요한 기능이 있는 경우 발견가능
    - 오타가 있는 경우 발견할 수 있다
- 시험 계획 : PMP, SRS를 바탕으로 시험 목표를 수립하고, 시험 대상 및 범위를 선정하는 활동
- 시험 설계 : 시험에 필요한 최적의 입력값(시험 사례)를 찾아내는 것 

### 소프트웨어 유지보수
- 사용자 요구 사항은 계속 변하기에 지속적인 변경을 수용할 수 있도록 만들어야하며, 적응성과 확장성을 고려하여 설계해야한다.
- 유지보수 정의
  - 수정 유지보수 : 잘못된 것 수정
  - 적응 유지보수 : 시스템을 새로운 환경에 적응
  - 완전 유지보수 : 새로운 기능을 추가
  - 예방 유지보수 : 미래의 시스템 관리를 위하는 것
- 역공학 : 원시 코드를이용해 원시 코드 이상의 추상화된 표현으로 나타내는 것으로 코드 분석 과정
  - 프로그램 구조, 자료구조, 모듈 간 관계 등 상세한 설계 정보를 도출
- 재공학 : 설계 정보를 끌어내는 것에 더해 보다 나은 시스템을 만드는 것
  - 새로운 기능 추가 및 성능 향상, 변경이 용이한 시스템으로 재구성 할 수 있다
- 유지보수 요청은 공식적 절차를 밟아 표준화된 방법으로 이루어져야하며 유지보수 요청서, 변경 요청서에 주로 이루어짐

### 형상관리
- 소프트웨어 형상 관리 : 개발단계에서 생산되는 산출물을 관리하는 모든 활동
- 개발 및 유지보수 과정에서 변경통제는 산출물을 관리하고 고품질 소프트웨어를 얻기위해 매우 중요
- 시스템 개발 및 유지보수 과정에서 발생하는 각종 결과물에 대한 계획, 개발, 운용 등을 종합해 시스템 형상을 만들고 이에 대한 변경을 체계적으로 관리, 제어하기 위한 활동
- 기준선 문서(Baselined Document)
  - 공식적인 검토 및 합의가 끝나 동결(서명되어 공식 승인된)된 문서
  - 개발을 위한 기준이되고, 공식적 변경 절차를 통해서만 변경
- 소프트웨어 형상 항목 : 형상 관리의 최소 단위
  - 형상 관리 대상이 되는 단위이며 작업 산출물을 의미
  - 서로 구별되고 이름이 붙여져 관리되어야 하는 기본 단위
- 형상 항목 식별
  - 형상 항목을 유일하게 식별하며 추적성을 제공해 접근 가능하도록 만드는 일
- 형상 통제
  - 변경 통제 프로세스
  
  ![ex_screenshot](/res/변경.JPG) 
  
### 소프트웨어 검증 및 확인
- 추적성 : 소프트웨어 개발 과정에서 요구사항과 이에 파생되는 다른 산출물 간의 관계를 관리하는 능력
- 확인 : 사용자 요구사항을 정확히 이해하여야 하며, 제대로 반영하여 구현해야 함.
  - 사용자 요구사항을 올바르게 이해하는 과정
  - 소프트웨어가 고객 기대와 요구 사항을 만족시킬 수 있도록 보장하는 활동
  - SRS에 요구사항을 올바르게 반영했는지, 소프트웨어가 요구사항을 만족하는지 점검 및 평가 작업 포함
  - 주요기법 : 시제품 개발 / 기술적 검토 / 시뮬레이션
- 검증 : 이해된 요구 사항을 소프트웨어에 제대로 반영하여 올바르게 제품을 만들어 갈 수 있도록 보장하는 활동
  - 소프트웨어가 지정된 기능을 정확히 수행할 수 있도록 단계별 점검을 통해 추적하는 활동
  - SRS에 정의되어 있는 특정 기능을 정확히 구현했는지 점검하는 활동
  - 현단계 산출물이 이전 단계 산출물과 일치하는지 검사하는 과정
- 확인과 검증은 프로젝트 관리의 중요 활동
- V&V 관리 활동 : 개발 생명주기에 걸쳐 수행
  - 요구사항이 타당성, 완전성, 정확성, 일관성이 충족되었는지 점검하는 것
