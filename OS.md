...
### 디스크 스케줄링
- 운영체제의 책임 중 하나는 효율적인 하드웨어 사용
  - 빠른 접근 시간과 높은 전송량을 제공해야 함을 의미
- 효율적인 스케줄링은 접근 시간과 대역폭을 모두 향상시킬 수 있다
- 프로세스가 입/출력을 필요로 할 때마다 운영체제에 시스템 호출을 한다
  - 입/출력, 디스크 주소, 메모리 주소, 전송될 섹터의 수
...
- 헤드가 다른곳으로 이동하기전 근처에 있는 요구들을 먼저 처리해주고 헤드를 이동하는 것
- 현재 헤드 위치에서 출발하여 탐색 시간이 가장 적은 요청
- 대기 중인 요청 중에서 현재의 헤드 위치가 가장 가까운 요청을 선택
- 몇 개의 요청들이 기아 상태가 발생할 수 있다 -> 큐 길이가 길어질수록 심해진다
![ex_screenshot](/res/os67.png)

4.3 SCAN 스케줄링
- 디스크 암이 디스크의 한 끝에서 시작해 다른 끝으로 이동하며 가는 길에 있는 요청을 모두 처리
- 다른 한쪽 끝에 도달하면 역방향으로 이동하며 오는 길에 있는 요청을 모두 처리
- 헤드는 디스크 양쪽을 계속해서 가르지르며 왕복
- 적용하기전 헤드의 현재 위치와 헤드의 진행 방향을 알아야 한다
![ex_screenshot](/res/os68.png)

4.4 C-SCAN 스케줄링
- 요청에 걸리는 시간을 존 더 균등하게 하기 위한 SCAN의 변형
- C-SAN은 한쪽 방향으로 헤드를 이동해가며 요청을 처리하지만 한쪽 끝에 다다르면 처음 시작했던 자리로 되돌아가 서비스를 시작
- 실린더들을 마지막 실린더가 처음 실린더와 맞닿은 원형 리스트로 간주
![ex_screenshot](/res/os69.png)

4.5 LOOK 스케줄링
- 헤드는 각 방향으로 가다가 방향에 아무도 기다리는 요청이 없으면 헤드의 이동 방향을 즉시 반대로 바꾼다
- SCAN, C-SCAN의 변형
- 한 방향으로 계속 움직이기 전 요구가 있는지 확인하기 때문에 LOOK, C-LOOK 스케줄링이라 한다
![ex_screenshot](/res/os70.png)

### 입출력 시스템

1.1 폴링
- 호스트와 입/출력 하드웨어 사이 프로토콜은 복잡하나 기본적 핸드 세이킹 개념은 간단
- 제어기는 상태 레지스터의 busy 비트를 통해 자신의 상태를 나타낸다
  - 바쁠 때는 busy 비트를 1로 설정하고 다음 명령을 받아들일 준비가 있는 경우 busy비트가 0으로 변경
- 호스트는 다음과 같은 방법으로 핸드세이킹을 통해 제어기와 협럭하면서 포트를 통해 출력을 쓴다
  - 호스트가 반복적으로 busy비트를 검사
  - 호스트가 명령 레지스터에 쓰기 비트를 설정하고 출력 레지스터에 출력할 바이트를 쓴다
  - 1. 호스트가 명령어 준비 완료 비트를 설정
  - 2. 제어기가 명령어 준비 완료 비트가 설정된 것을 알아 차렸을 때 자신의 busy 비트를 설정
  - 3. 제어기는 명령어 레지스터를 읽고 write 명령어임을 알게된다, 출력 레지스터를 읽어 해당 바이트를 가져와 해당 하드웨어 장치로 출력
  - 4. 제어기는 멸영어 준비 완료 비트를 소거하고 입/출력이 끝났음을 알리기 위해 busy 비트를 소거
- 매 바이트마다 반복되며 1단계에서 호스트는 폴링하게 된다

1.2 인터럽트
- CPU 하드웨어는 인터럽트 라인이라 불리는 선을 갖고 CPU는 매 명령어를 끝내고 다음 명령어를 수행하기 전에 인터럽트 요청 라인을 검사
  - 입/출력 하드웨어 제어기가 이 요청 라인에 신호를 보낸다
  - CPU가 각종 레지스터 값과 상태 정보를 저장한 다음 메모리 상 인터럽트 핸들러 루틴으로 이동
  - 인터럽트 핸들러는 인터럽트의 발생 원인을 조사하고 필요한 작업을 수행한 후 CPU를 인터럽트 이전 상태로 되돌리는 명령을 수행한다
  - 핸들러는 입/출력 장치를 서비스함으로써 인터럽트를 처리해 준다
- 대부분의 CPU 들은 두 종료의 인터럽트 요청 라인을 가진다
  - 마스크 불가 인터럽트 : 회복 불가능한 메모리 에러와 같은 이벤트를 위해 사용
  - 마스크 가능 인터럽트 : 필요 시 인터럽트 기능을 잠시 중단시켜 놓을 수 있는 기능으로 장치 제어기가 서비스를 요청하기 위해 사용
    - 인터럽트 되어서는 안되는 주요 명령 시퀀스를 수행하기 전 CPU가 끌 수 있다
- 보통 주소라는 하나의 작은 정수를 받아들이는데 정수는 특정 인터럽트 핸들링 루틴을 선택하기 위해 사용되며 대부분 구조에서 주소는 인터럽트 벡터라 불리는 테이블의 오프셋으로 사용된다
- 컴퓨터는 인터럽트 백터 내에 있는 주소들보다 더 많은 수 장치를 갖고 있다
  - 인터럽트 사슬 기술을 사용해 문제를 해결
- 인터럽트 사슬화
  - 인터럽트 벡터의 각 원소들이 여러 인터럽트 핸들러로 이루어진 리스트의 헤더를 가리키고 있다
  - 인터럽트가 발생하면 해당 핸들러가 찾아질 때까지 리스트 상의 핸들러들을 하나씩 검사하게 됨
- 인터럽트는 우선순위 수준의 구현을 가능하게 한다
  - CPU가 모든 낮은 우선순위 인터럽트를 마스크 오프시키지 않더라도 자동적으로 높은 우선순위 인터럽트가 낮은 인터럽트 실행을 선점할 수 있게 한다

1.3 DMA
- CPU가 상태 비트를 반복 검사하며 1바이트씩 옮기는 입/출력 방식을 PIO라 한다
- 많은 컴퓨터들은 CPU의 PIO 작업 중 일부를 DMA 제어기라고 불린른 특수 처리기에 위임함으로써 CPU의 일을 줄여준다
- DMA 전송을 시작시키기 위해 호스트는 메모리에 DMA 명령 블록을 쓴다
  - 이 블록에는 전송할 자료가 있는 꽀의 포인터와 전송할 장소에 대한 포인터, 전송될 바이트 수를 기록해 놓는다
- CPU는 DMA 명령 블록의 주소를 DMA에 알려주고 자신은 다른일 수행
- DMA는 CPU의 도움 없이도 자신이 직접 버스를 통해 DMA 명령 블록을 접근해 입/출력 수행

- DMA 제어기와 장치 제어기간 핸드 세이킹은 DMA-request와 DMA-acknowledge라 불리는 두개의 선을 통해 수행
  - 장치 제어기는 전송할 자료가 생기면 DMA-request 선에 신호를 보냄
  - 신호를 받으면 DMA 제어기가 메모리버스를 얻어 거기게 원하는 주소를 올려놓고 DMA-acknowledge 선에 신호를 보냄
  - 장치 제어기 DMA-acknowledge 신호를 받으면 제어기는 한 워드를 메모리로 전송하고 DMA-request 신호를 제거한다
  - 전송이 끝나면 DMA 제어기는 CPU에게 인터럽트 건다
- DMA가 메모리 버스를 점유 중이면 CPU는 주 캐시와 보조 캐시에 있는 자료를 접근할 수 있으나 일시적으로 주 메모리의 자료에 접근하지 못한다
- 사이클 스틸링은 CPU 속도를 저하시키지만 입/출력 작업을 DMA로 넘기는 것은 시스템 성능을 향상시킨다
- DMA를 사용할 때 물리적 주소를 사용하지만 DVMA(직접 가상 메모리 접근)을 사용하기도 한다
  - CPU나 메모리의 개입 없이도 가상 주소로 두개의 메모리 매핑 장치간에 자료를 전송할 수 있게 된다
